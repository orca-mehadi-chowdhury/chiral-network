<script lang="ts"> 
 ...
 // Bar or Line chart toggle
  ...
  onMount(async () => {
    ...
    // Seed the seen-hash set with anything already present
    try {
  ...
  async function updateMiningStats() {
    try {
        ...
        try {
          ...
          if (hashRateFromLogs > 0) {
            ...
            if (blocksFound > $miningState.blocksFound) {
              ...
              // Visualization handled by backend-driven fetch
            }
      ...
      if ($miningState.isMining) {
        ...
        // No simulation here; recent blocks are pulled from backend
      }
    } catch (e) {
  ...
  async function appendNewBlocksFromBackend() {
    try {
      ...
      const lookback = 2000;
      const limit = 50;
      const blocks = await invoke('get_recent_mined_blocks', {
        address: $etcAccount.address,
        lookback,
        limit
